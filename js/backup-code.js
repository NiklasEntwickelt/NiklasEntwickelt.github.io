...
window.addEventListener("DOMContentLoaded",() => {
    ...
    if(localStorage.getItem("timeline") != null) {
        try {
            ...
            createToast("saveNotification","Loading data from Save","")
            removeAllEntries();
...
const saveData =  (cause) => {
    ...
    switch(cause) {
        case "autosave":
            console.log("Autosaving timeline in Localstorage...")
        break;
        default:
    ...
    try {
        ...
        document.querySelector("#exportPDF").onclick = () => {  
            if(!document.querySelector("#addEntry").hasAttribute("disabled"))
                ...
                createToast("warning","Button currently disabled");
            }
...
const resetTimeline = () => {
    removeAllEntries();
    //get default tasks to be added

    let position = 1;
    for(let child of document.querySelector("#default-timeline").children) {
        if(position == 1) {position++;continue};
        addEntry(child.children[0].innerText,child.children[1].innerText,child.children[2].innerText,"last",true);
        position++;
    }
}
...
const exportTasksAsText = () => {
    ...
    textExport.innerHTML +=  `<div id="textExportWrapper" class="overflow-hidden" style="padding: 10px;margin:25px;border-radius:10px;color: white;background-color:${exportBackgroundColor}">`;
    ...
    switch(cell){
        ...
        case 1:
            textExportWrapper.innerHTML += `<span style="text-shadow: ${timeCellTextShadowColor};color: ${timeCellTextColor};background: ${timeCellColor};border: 1px solid ${timeCellBorderColor};border-radius:10px;padding: 2px;margin: 50px 0px 50px 0px"> ${value}  </span>`;
        break;
            
        //this is the task cell
        case 2:
            textExportWrapper.innerHTML += `<span style="text-shadow: ${taskCellTextShadowColor};color: ${taskCellTextColor};background: ${taskCellColor};border: 1px solid ${taskCellBorderColor};border-radius:10px;padding: 2px;margin: 50px 0px 50px 0px"> ${value} </span>`;
            break;
            
            //this is the comment cell
            case 3:
                if(value.length >= 1) {
                    textExportWrapper.innerHTML += `<span style="text-shadow: ${commentCellTextShadowColor};color: ${commentCellTextColor};background: ${commentCellColor};border: 1px solid ${commentCellBorderColor};border-radius:10px;padding: 2px;margin: 50px 0px 50px 0px">Anmerkung: ${value} </span></div>`;
...
function setClipboard(text) {
    ...
    navigator.clipboard.write(data).then(
        () => {
        /* Success */
            createToast("warning","Successfully send data to Clipboard","")
        },
        () => {
        /* failure */
            createToast("warning","Failed to send data to Clipboard","")
        }
    );
}
...
const startSaveScheduler = async () =>  {
    ...
    const SaveScheduler = async () => {
        await wait(1000*60)
        saveData("autosave");
        createToast("autosave","Saving Your Data...","")
        SaveScheduler()
    }
...
let timelineSortBuffer = new Map();

const sortTimelineByTime = () => {
    timelineSortBuffer.clear();
    ...
    for (let child of timeline.children) {
        ...
        for(let subchild of child.children) {
            //TR => Element containers, time, text, comment, button
            if(subchild.classList.contains("timeline-item-time"))
                 timelineSortBuffer.set('Test', {value:"1",value2:"2"})
        }
    }
